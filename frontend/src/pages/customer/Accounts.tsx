...
import { Copy, CheckCheck, Landmark, TrendingUp, TrendingDown, ArrowLeftRight, ChevronDown, ChevronUp, Banknote, MinusCircle, Clock } from 'lucide-react'
...
import DepositDenomModal from '../../components/ui/DepositDenomModal'
import type { DenomValues } from '../../components/ui/DepositDenomModal'
import { accountsApi } from '../../api/accounts'
import { transactionsApi } from '../../api/transactions'
import { depositsApi } from '../../api/deposits'
import type { DepositRequestOut } from '../../api/deposits'
import { formatINR, formatDateTime, maskAccount, getCategoryIcon, getCategoryColor } from '../../utils/formatters'
import { useAuth } from '../../contexts/AuthContext'
import type { Account, Transaction } from '../../types'
import clsx from 'clsx'

type ActionPanel = 'mini' | 'withdraw' | null
...
function AccountCard({ account }: { account: Account }) {
  ...
  const [showDepositDenom, setShowDepositDenom] = useState(false)
  const [pendingDeposit, setPendingDeposit] = useState<{ denoms: DenomValues; total: number } | null>(null)
  const [depositRequests, setDepositRequests] = useState<DepositRequestOut[]>([])
  // Withdraw flow
  const [showConfirm, setShowConfirm] = useState(false)
  const [showOtp, setShowOtp] = useState(false)
  const [pendingAction, setPendingAction] = useState<'withdraw' | null>(null)
  const [actionAmount, setActionAmount] = useState('')
  const [actionLoading, setActionLoading] = useState(false)

  useEffect(() => {
    depositsApi.myRequests()
      .then(reqs => setDepositRequests(reqs.filter(r => r.account_id === account.id)))
  ...
  function togglePanel(panel: 'withdraw') {
    setActivePanel(prev => (prev === panel ? null : panel))
    setActionAmount('')
  }

  // Deposit: show denomination modal, then confirm + OTP
  function handleDepositClick() {
    setShowDepositDenom(true)
  }

  function handleDenomConfirm(denoms: DenomValues, total: number) {
    setPendingDeposit({ denoms, total })
    setShowDepositDenom(false)
    setShowConfirm(true)
    setPendingAction(null)
  }

  async function executeDepositRequest() {
    if (!pendingDeposit) return
    setActionLoading(true)
    try {
      const req = await depositsApi.createRequest(account.id, pendingDeposit.denoms)
  ...
  // Withdraw flow (still instant, requires OTP)
  ...
  return (
    <ScrollReveal>
        ...
        <div className="bg-white">
          ...
          <div className="grid grid-cols-3 gap-2 p-4">
            ...
            >
              ...
              {activePanel === 'mini' ? 'Hide' : 'Statement'}
            </button>
            <button
              onClick={handleDepositClick}
              disabled={account.status !== 'active'}
              className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed border-gray-200 text-gray-600 hover:border-emerald-300 hover:text-emerald-700 hover:bg-emerald-50"
            >
              <Banknote size={13} />
              Deposit
            </button>
          ...
          {/* Pending deposit requests badge */}
          {depositRequests.filter(r => r.status === 'pending').length > 0 && (
            <div className="mx-4 mb-3 flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
              <Clock size={12} className="shrink-0" />
              {depositRequests.filter(r => r.status === 'pending').length} deposit request(s) pending admin approval
            </div>
          )}

          {/* Animated panels — withdraw only */}
          <AnimatePresence>
            {activePanel === 'withdraw' && (
              <motion.div
                key="withdraw"
              ...
              >
                <div className="mx-4 mb-4 p-4 rounded-xl border bg-red-50 border-red-200">
                  <p className="text-xs font-semibold mb-3 text-red-700">Withdraw from this account</p>
      ...
      {/* Deposit denomination modal */}
      <DepositDenomModal
        open={showDepositDenom}
        accountLabel={`••••${account.account_number.slice(-4)} (${account.account_type})`}
        onConfirm={handleDenomConfirm}
        onCancel={() => setShowDepositDenom(false)}
      />

      {/* Confirm modal — deposit request or withdraw */}
      <TxnConfirmModal
        open={showConfirm}
        title={pendingDeposit ? 'Confirm Deposit Request' : 'Confirm Withdrawal'}
        rows={pendingDeposit ? [
          { label: 'Account', value: `••••${account.account_number.slice(-4)} (${account.account_type})` },
          { label: '₹100 notes', value: `${pendingDeposit.denoms.notes_100} × ₹100 = ₹${(pendingDeposit.denoms.notes_100 * 100).toLocaleString('en-IN')}` },
          { label: '₹200 notes', value: `${pendingDeposit.denoms.notes_200} × ₹200 = ₹${(pendingDeposit.denoms.notes_200 * 200).toLocaleString('en-IN')}` },
          { label: '₹500 notes', value: `${pendingDeposit.denoms.notes_500} × ₹500 = ₹${(pendingDeposit.denoms.notes_500 * 500).toLocaleString('en-IN')}` },
          { label: 'Total', value: formatINR(pendingDeposit.total), highlight: true },
        ] : [
          { label: 'Account', value: `••••${account.account_number.slice(-4)} (${account.account_type})` },
          { label: 'Type', value: 'Withdrawal' },
          { label: 'Amount', value: formatINR(parseFloat(actionAmount) || 0), highlight: true },
        ]}
        onConfirm={() => { setShowConfirm(false); setShowOtp(true) }}
        onCancel={() => { setShowConfirm(false); setPendingAction(null); setPendingDeposit(null) }}
      />
      ...
      <OtpModal
        ...
        onVerified={() => {
          setShowOtp(false)
          if (pendingDeposit) { executeDepositRequest() }
          else if (pendingAction === 'withdraw') { executeWithdraw() }
        }}
        onCancel={() => { setShowOtp(false); setPendingAction(null); setPendingDeposit(null) }}
      />
    </ScrollReveal>
  )
}
